...
DOCKER_IMAGE_NAME = 'subscriber:latest'
...
def test_pip_packages(host):
    ...
    :return:
    """
    expected_python_pkg = ['google-api-python-client', 'configparser', 'google-cloud-pubsub',
                           'python-dateutil', 'google-cloud-language']
...
def test_application_artifacts(host):
    ...
    application_directory = host.file('/subscriber/')
    assert application_directory.exists

    # TEST: CHECK IF THE CREDENTIALS DIRECTORY EXISTS and is not empty
    credentials_directory = host.file('/subscriber/credentials')
    assert credentials_directory.exists
    crendentials_dir_contents = host.run('ls /subscriber/credentials').stdout
    assert crendentials_dir_contents

    # TEST: CODE ARTIFACTS ARE COPIED TO THE WORKDIR
    work_dir_contents = host.run('ls /subscriber/').stdout
    expected_work_dir_contents = ['__init__.py', 'app.conf', 'requirements.txt', 'tweet_subscriber.py']
...
def test_env_var(host):
    ...
    assert str(python_path).strip() == '/subscriber'
